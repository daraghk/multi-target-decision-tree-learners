...
fn test_decision_tree_for_iris() {
    ...
    let test_set = read_csv_data("./../common/data-files/iris_test.csv");
...
fn test_decision_tree_for_synthetic() {
    let data_set = read_csv_data("./../common/data-files/synthetic_1.csv");
    let split_finder = SplitFinder::new(SplitMetric::Variance);
    let tree = DecisionTree::new(data_set, split_finder, 3, false);
    let boxed_tree = Box::new(tree.root);
    let test_set = read_csv_data("./../common/data-files/synthetic_1.csv");
    let accuracy = calculate_accuracy(&test_set, &boxed_tree);
    assert_eq!(accuracy, 1.0);
}
...
fn test_decision_tree_for_digits() {
    let data_set = read_csv_data("./../common/data-files/digits_train.csv");
    let split_finder = SplitFinder::new(SplitMetric::Variance);
    let tree = DecisionTree::new(data_set, split_finder, 10, false);
    let boxed_tree = Box::new(tree.root);
    let test_set = read_csv_data("./../common/data-files/digits_test.csv");
    let accuracy = calculate_accuracy(&test_set, &boxed_tree);
...
fn test_decision_tree_for_wine() {
    let data_set = read_csv_data("./../common/data-files/wine_train.csv");
    let split_finder = SplitFinder::new(SplitMetric::Variance);
    let tree = DecisionTree::new(data_set, split_finder, 3, false);
    let boxed_tree = Box::new(tree.root);
    let test_set = read_csv_data("./../common/data-files/wine_test.csv");
    let accuracy = calculate_accuracy(&test_set, &boxed_tree);
...
fn test_decision_tree_for_covtype() {
    ...
    let boxed_tree = Box::new(tree.root);
    let test_set = read_csv_data("./../common/data-files/covtype_test.csv");
    let accuracy = calculate_accuracy(&test_set, &boxed_tree);
    println!("{}", accuracy);
    assert!(accuracy > 0.90)
}
